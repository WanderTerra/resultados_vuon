...
const BlocoWO = () => {
    ...
    useEffect(() => {
        if (!dashboardData?.wo) { setAcoesPorBloco([]); return; }
        ...
        fetch(`${API_ENDPOINTS.blocoAcoes('wo')}${params.toString() ? '?' + params.toString() : ''}`, { headers: { Authorization: `Bearer ${token}` } })
    ...
    }, [dashboardData?.wo, filters.startDate, filters.endDate, filters.groupBy]);
