        ...
        <div className="space-y-6 px-4">
            ...
            <section>
                <div className="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <p className="text-slate-500">61 a 90 dias de atraso</p>
                    </div>
                    <div className="flex items-center gap-2">
                        <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-sm font-medium">
                            {(dashboardData?.bloco1?.spins || 0).toLocaleString('pt-BR')} Spins ({getSpinsPeriodoLabel()})
                        </span>
                        {dashboardData?.bloco1?.spinsLastDayDate && (
                            <span className="px-3 py-1 bg-slate-100 text-slate-700 rounded-full text-sm font-medium">
                                {(dashboardData?.bloco1?.spinsLastDay || 0).toLocaleString('pt-BR')} Spins ({formatPtBR(dashboardData.bloco1.spinsLastDayDate)})
                            </span>
                        )}
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    <ChartContainer 
                        ...
                        compareData={compareData?.bloco1?.acionadosXCarteira}
                    />
                    <ChartContainer 
                        title="Acionados x Alô" 
                        data={dashboardData?.bloco1?.acionadosXAlo || []} 
                        compareData={compareData?.bloco1?.acionadosXAlo}
                    />
                    <ChartContainer 
                        title="Alô x CPC" 
                        data={dashboardData?.bloco1?.aloXCpc || []} 
                        compareData={compareData?.bloco1?.aloXCpc}
                    />
                    <ChartContainer 
                        title="CPC x CPCA" 
                        data={dashboardData?.bloco1?.cpcXCpca || []} 
                        compareData={compareData?.bloco1?.cpcXCpca}
                    />
                    <ChartContainer 
                        title="CPCA x Acordos" 
                        data={dashboardData?.bloco1?.cpcaXAcordos || []} 
                        compareData={compareData?.bloco1?.cpcaXAcordos}
                    />
                    <ChartContainer 
                        title="Acordos x Pagamentos" 
                        data={dashboardData?.bloco1?.acordosXPagamentos || []} 
            ...
            <section>
                <div className="mb-6">
                    ...
                    <p className="text-slate-500">Evolução mensal de clientes únicos e acordos no Bloco 1 (61 a 90 dias de atraso)</p>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    <div className="lg:col-span-2 flex justify-center">
                        <div className="w-full lg:w-1/2">
                            <ClientesVirgensChart bloco={1} startDate={filters.startDate} endDate={filters.endDate} />
                        </div>
                    </div>
                </div>
            </section>
