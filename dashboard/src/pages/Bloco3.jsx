...
const Bloco3 = () => {
    ...
    useEffect(() => {
        if (!dashboardData?.bloco3) { setAcoesPorBloco([]); return; }
        ...
        fetch(`${API_ENDPOINTS.blocoAcoes(3)}${params.toString() ? '?' + params.toString() : ''}`, { headers: { Authorization: `Bearer ${token}` } })
            .then((r) => r.ok ? r.json() : []).then((data) => setAcoesPorBloco(Array.isArray(data) ? data : [])).catch(() => setAcoesPorBloco([])).finally(() => setAcoesLoading(false));
    }, [dashboardData?.bloco3, filters.startDate, filters.endDate, filters.groupBy]);
        ...
        <div className="space-y-6 px-4">
                ...
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-sm font-medium">
                            {(dashboardData?.bloco3?.spins || 0).toLocaleString('pt-BR')} Spins ({getSpinsPeriodoLabel()})
                        </span>
                        {dashboardData?.bloco3?.spinsLastDayDate && (
                            <span className="px-3 py-1 bg-slate-100 text-slate-700 rounded-full text-sm font-medium">
                                {(dashboardData?.bloco3?.spinsLastDay || 0).toLocaleString('pt-BR')} Spins ({formatPtBR(dashboardData.bloco3.spinsLastDayDate)})
                            </span>
                        )}
                    </div>
                </div>
            ...
            <section>
                <div className="mb-6">
                    ...
                    <p className="text-slate-500">Evolução mensal de clientes únicos e acordos no Bloco 3 (181 a 360 dias de atraso)</p>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    <div className="lg:col-span-2 flex justify-center">
                        <div className="w-full lg:w-1/2">
                            <ClientesVirgensChart bloco={3} startDate={filters.startDate} endDate={filters.endDate} />
                        </div>
                    </div>
                </div>
            </section>
